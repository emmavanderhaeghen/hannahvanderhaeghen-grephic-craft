    ...
    public function init()
    {
        ...
        Event::on(Entry::class, Entry::EVENT_AFTER_SAVE, function(ModelEvent $event) {
            ...
            if ($event->isNew
                ...
                $globalSet = Craft::$app->globals->getSetByHandle('commerceMails');
        ...
        Event::on(Elements::class, Elements::EVENT_BEFORE_SAVE_ELEMENT, function(ElementEvent $event) {
            ...
            if ($element instanceof craft\commerce\elements\Product) {
                $categoryGroupHandles = [];

                foreach ($element->getFieldLayout()->getFields() as $field) {
                    if ($field instanceof craft\fields\Categories) {
                        if ($element[$field->handle]->count()) {
                            $categoryGroup = Craft::$app->categories->getGroupByUid(str_replace('group:', '', $field->getSettings()['source']));
                            array_push($categoryGroupHandles, $categoryGroup->handle);
                        }
                    }
                }

                $categoryGroupHandles = array_unique($categoryGroupHandles);
                $element->setFieldValue('filterGroups', implode(',', $categoryGroupHandles));
            }
        });
